...
export const PomodoroOverlay: React.FC<PomodoroOverlayProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950 text-slate-100 flex flex-col items-center justify-between p-6 sm:p-10 select-none overflow-y-auto backdrop-blur-md animate-in fade-in duration-200">
      {/* Top Header / Lock Indicator & Session Badge */}
      <div className="w-full max-w-4xl flex items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-2xl bg-blue-500/10 border border-blue-500/30 text-blue-400 flex items-center gap-2">
          ...
          <span className="text-xs text-slate-400 hidden sm:inline">• UI Terkunci untuk Mencegah Distraksi</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-slate-900/80 hover:bg-rose-950/60 border border-slate-800 hover:border-rose-800 text-slate-400 hover:text-rose-300 text-xs font-semibold flex items-center gap-1.5 transition-colors"
      ...
      <div className="my-auto w-full max-w-2xl flex flex-col items-center text-center space-y-8 py-6">
        ...
        <div className="space-y-3">
          {subject && (
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-xs font-semibold">
              <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: subject.color }} />
              <span className="text-slate-300">{subject.code} — {subject.name}</span>
            </div>
          )}
          ...
          <p className="text-xs text-slate-400 flex items-center justify-center gap-2 font-medium">
            ...
            <span>Sesi: <strong className="text-slate-200">{session.sessionType}</strong></span>
            <span>•</span>
        ...
        <div className="inline-flex p-1.5 rounded-2xl bg-slate-900/90 border border-slate-800 gap-1.5">
          <button
            onClick={() => {
              setTimerMode('FOCUS');
              setIsRunning(false);
        ...
        {timerMode === 'FOCUS' && (
          <div className="flex items-center justify-center gap-2 text-xs">
            <span className="text-slate-400 font-medium mr-1">Durasi Fokus:</span>
        ...
        <div className="flex items-center justify-center gap-4 pt-2">
          ...
          <button
            ...
            className={`px-8 py-4 rounded-2xl font-bold text-sm flex items-center gap-3 shadow-xl transition-all active:scale-95 ${
              isRunning
          ...
          <button
            ...
            className="px-5 py-4 rounded-2xl bg-emerald-600/20 hover:bg-emerald-600 border border-emerald-500/40 hover:border-emerald-500 text-emerald-300 hover:text-white font-bold text-xs flex items-center gap-2 transition-all shadow-lg shadow-emerald-900/20 active:scale-95"
          ...
          {onOpenQuickQuiz && (
            <button
              ...
              className="px-4 py-4 rounded-2xl bg-indigo-600/30 hover:bg-indigo-600 border border-indigo-500/40 hover:border-indigo-500 text-indigo-200 hover:text-white font-bold text-xs flex items-center gap-2 transition-all shadow-lg shadow-indigo-900/20 active:scale-95"
      ...
      <div className="w-full max-w-2xl border-t border-slate-800/80 pt-4 text-center space-y-2">
        <div className="inline-flex items-center gap-2 text-xs text-blue-400 font-semibold">
          <Sparkles className="w-3.5 h-3.5 text-amber-400" />
          <span>Tips Study & Legal Mindset</span>
        </div>
        <p className="text-xs text-slate-300 italic max-w-xl mx-auto leading-relaxed">
          "{FOCUS_QUOTES[quoteIndex]}"
        </p>
        {session.notes && (
          <p className="text-[11px] text-slate-500 max-w-md mx-auto truncate pt-1">
            📌 Catatan: <span className="text-slate-400">{session.notes}</span>
          </p>
        )}
      </div>
    </div>
